...
namespace ml
{
	...
	bool PropertyDrawer<Entity>::operator()(const String & label, pointer & value) const
	{
		...
		// Popup
		const String button_label { ("{0}##NewButton##{1}"_s).format(label, info().name.str()) };
		const String popup_label { ("Create {1}##{0}##Popup"_s).format(label, info().name.str()) };
		...
		if (ImGui::BeginPopupModal(popup_label.c_str(), 0, ImGuiWindowFlags_AlwaysAutoResize))
		{
			...
			// Name
			ImGui::InputText(
				("Name##" + info().name.str() + "##" + label).c_str(),
				name,
				ML_ARRAYSIZE(name)
			);
	...
	bool PropertyDrawer<Font>::operator()(const String & label, pointer & value) const
	{
		...
		if (ImGui::BeginPopupModal(popup_label.c_str(), 0, ImGuiWindowFlags_AlwaysAutoResize))
		{
			...
			// Path
			ImGui::InputText(
				("##Path##" + info().name.str() + "##" + label).c_str(),
				asset_path,
				ML_MAX_PATH
			);
			ImGui::SameLine();
