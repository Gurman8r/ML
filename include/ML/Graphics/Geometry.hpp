...
namespace ml
{
	...
	namespace geo
		...
		struct triangle_static final : public impl::static_mesh<3, 3>
		{
			...
			{
				Vertex { {  0.0f,  0.5f, 0.0f }, vec4::one(), { 0.5f, 1.0f } },
				Vertex { {  0.5f, -0.5f, 0.0f }, vec4::one(), { 1.0f, 0.0f } },
				Vertex { { -0.5f, -0.5f, 0.0f }, vec4::one(), { 0.0f, 0.0f } },
			};
		...
		struct quad_static final : public impl::static_mesh<4, 6>
		{
			static constexpr vertices_t vertices
			{
				Vertex { { +1.0f, +1.0f, 0.0f }, vec4::one(), { 1.0f, 1.0f } },
				Vertex { { +1.0f, -1.0f, 0.0f }, vec4::one(), { 1.0f, 0.0f } },
				Vertex { { -1.0f, -1.0f, 0.0f }, vec4::one(), { 0.0f, 0.0f } },
				Vertex { { -1.0f, +1.0f, 0.0f }, vec4::one(), { 0.0f, 1.0f } },
			};
			static constexpr indices_t indices
			{
				0, 1, 3,
		...
		struct cube_static final : public impl::static_mesh<24, 36>
		{
			static constexpr vertices_t vertices
			{
				Vertex { { +0.5f, +0.5f, -0.5f }, vec4::one(), { 1.0f, 1.0f } },
				Vertex { { +0.5f, -0.5f, -0.5f }, vec4::one(), { 1.0f, 0.0f } },
				Vertex { { -0.5f, -0.5f, -0.5f }, vec4::one(), { 0.0f, 0.0f } },
				Vertex { { -0.5f, +0.5f, -0.5f }, vec4::one(), { 0.0f, 1.0f } },
				Vertex { { +0.5f, +0.5f, +0.5f }, vec4::one(), { 1.0f, 1.0f } },
				Vertex { { +0.5f, -0.5f, +0.5f }, vec4::one(), { 1.0f, 0.0f } },
				Vertex { { -0.5f, -0.5f, +0.5f }, vec4::one(), { 0.0f, 0.0f } },
				Vertex { { -0.5f, +0.5f, +0.5f }, vec4::one(), { 0.0f, 1.0f } },
		...
		struct skybox_static final : public impl::static_mesh<36, 0>
		{
			static constexpr vertices_t vertices
			{
				Vertex { { -1.0f,  1.0f, -1.0f }, vec4::one(), vec2::zero() },
				Vertex { { -1.0f, -1.0f, -1.0f }, vec4::one(), vec2::zero() },
				Vertex { {  1.0f, -1.0f, -1.0f }, vec4::one(), vec2::zero() },
				Vertex { {  1.0f, -1.0f, -1.0f }, vec4::one(), vec2::zero() },
				Vertex { {  1.0f,  1.0f, -1.0f }, vec4::one(), vec2::zero() },
