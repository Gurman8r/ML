...
namespace ml
{
	...
	// Will insert value if it does not exist, then returns const reference to the value.
	template <
	...
	> struct Cache final : public I_NonNewable
	{
		...
		{
			const_iterator it;
			return (((it = m_data.find(id)) != m_data.end()) ? &it->second : nullptr);
		}

		inline const_reference operator()(const key_type & key, const_reference value)
		{
			const hash_t id { Hash()(key) };
			const_pointer temp = (*this)[id];
			return ((temp) ? (*temp) : m_data.insert({ id, value }).first->second);
		}
