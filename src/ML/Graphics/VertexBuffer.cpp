...
namespace ml
{
	...
	const VertexBuffer & VertexBuffer::bufferData(voidptr_t data, uint32_t size) const
	{
		if (*this)
		{
			m_data	= data;
	...
	const VertexBuffer & VertexBuffer::bufferSubData(voidptr_t data, uint32_t size, uint32_t offset) const
	{
		if (*this)
		{
			m_data	= data;
